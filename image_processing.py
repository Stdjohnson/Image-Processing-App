...
#
def intensify(pixel):
    red_lighten = 10 + int(pixel[0])
    green_lighten = 10 + int(pixel[1])
    blue_lighten = 10 + int(pixel[2])
    if red_lighten >= 255:
        red_lighten = 255
    if green_lighten >= 255:
        green_lighten = 255
    if blue_lighten >= 255:
        blue_lighten = 255
    return (red_lighten, green_lighten, blue_lighten)

def detensify(pixel):
    red_darken = int(pixel[0]) - 10
    green_darken = int(pixel[1]) - 10
    blue_darken = int(pixel[2]) - 10
    if red_darken < 0:
        red_darken = 0
    if green_darken < 0:
        green_darken = 0
    if blue_darken < 0:
        blue_darken = 0
    return (red_darken, green_darken, blue_darken)
################### IMAGE OPERATIONS ######################
